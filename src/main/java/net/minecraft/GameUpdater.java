...
//            Pack200.class.getSimpleName();
    ...
    protected void downloadJars(final String path) throws Exception {
        ...
        for (int j = 0; j < this.urlList.length; ++j) {
            if (skip[j]) {
                this.percentage = percentage + fileSizes[j] * 45 / this.totalSizeDownload;
            } else {
                try {
                ...
                while (downloadFile) {
                    ...
                    while ((bufferSize = inputstream.read(buffer, 0, buffer.length)) != -1) {
                        ...
                        fileSize += bufferSize;
                        this.percentage = percentage + this.currentSizeDownload * 45 / this.totalSizeDownload;
    ...
    protected void extractNatives(final String path) throws Exception {
        ...
        final File file = new File(path + nativeJar);
        if (!file.exists()) {
            return;
        }
